...
class LetLeedDaylightOptionOneFly(luigi.WrapperTask):
    # global parameters
...
def start(project_folder, user_values, workers):
    ...
    if 'simulation_folder' not in input_params or not input_params['simulation_folder']:
        if 'simulation_id' not in input_params or not input_params['simulation_id']:
            simulation_id = 'leed_daylight_option_one_%d' % int(round(time.time(), 2) * 100)
        else:
    ...
    summary = luigi.build(
        [LetLeedDaylightOptionOneFly(_input_params=input_params)],
