...
class RunTwoPhaseDaylightCoefficient(QueenbeeTask):
    ...
    def requires(self):
        return {'AddApertureGroupBlinds': AddApertureGroupBlinds(_input_params=self._input_params)}
...
class LeedDaylightOptionOne(QueenbeeTask):
    ...
    @property
    def blind_postprocess(self):
        return 'states'

    shade_transmittance = luigi.Parameter(default='0.02')
    ...
    @property
    def __script__(self):
        return pathlib.Path(__file__).parent.joinpath('scripts', 'leed_daylight_option_one.py').resolve()
    ...
    def output(self):
        return {
            'leed_summary': luigi.LocalTarget(
                pathlib.Path(self.execution_folder, 'leed_summary').resolve().as_posix()
            )
        }
    ...
    def output_artifacts(self):
        return [
            {
                'name': 'leed-summary', 'from': 'leed_summary',
                'to': pathlib.Path(self.execution_folder, 'leed_summary').resolve().as_posix(),
                'optional': False,
                'type': 'folder'
            }]
    ...
    def input_parameters(self):
        return {
            'grid_filter': self.grid_filter,
            'blind_postprocess': self.blind_postprocess,
            'shade_transmittance': self.shade_transmittance}
...
class CreateVisualization(QueenbeeTask):
    ...
    @property
    def pass_fail(self):
        value = pathlib.Path(self.input()['LeedDaylightOptionOne']['leed_summary'].path, 'pass_fail')
        return value.as_posix() if value.is_absolute() \
            else pathlib.Path(self.initiation_folder, value).resolve().as_posix()
    ...
    def requires(self):
        return {'RunTwoPhaseDaylightCoefficient': RunTwoPhaseDaylightCoefficient(_input_params=self._input_params), 'LeedDaylightOptionOne': LeedDaylightOptionOne(_input_params=self._input_params)}
