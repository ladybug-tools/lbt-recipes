...
class CreateVisMetadata(QueenbeeTask):
    ...
    def command(self):
        return 'honeybee-radiance-postprocess post-process leed leed-daylight-option-one-vis-metadata --output-folder "{output_folder}"'.format(output_folder=self.output_folder)
...
class CreateVsf(QueenbeeTask):
    ...
    def command(self):
        return 'honeybee-display model-to-vis model.hbjson --color-by {color_by} --{color_visibility}-color-by --{wireframe} --{attr_format}-attr --room-attr "{room_attr}" --face-attr "{face_attr}" --grid-display-mode {grid_display_mode} --{grid_visibility}-grid --grid-data input_data --grid-data-display-mode {grid_data_display_mode} --active-grid-data "{active_grid_data}" --output-format {output_format} --output-file model_vis.{output_format}'.format(attr_format=self.attr_format, grid_data_display_mode=self.grid_data_display_mode, output_format=self.output_format, active_grid_data=self.active_grid_data, wireframe=self.wireframe, face_attr=self.face_attr, color_visibility=self.color_visibility, color_by=self.color_by, room_attr=self.room_attr, grid_visibility=self.grid_visibility, grid_display_mode=self.grid_display_mode)
...
class _DaylightOptionOneVisualization_d1cc8deaOrchestrator(luigi.WrapperTask):
