...
class CreateDirectSky(QueenbeeTask):
    ...
    def command(self):
        return 'honeybee-radiance sky mtx sky.epw --name sky --north {north} --sky-type {sky_type} --{cumulative} --{sun_up_hours} --{output_type} --output-format {output_format} --sky-density {sky_density}'.format(cumulative=self.cumulative, sun_up_hours=self.sun_up_hours, sky_type=self.sky_type, north=self.north, output_format=self.output_format, sky_density=self.sky_density, output_type=self.output_type)
...
class PrepareMultiphase(QueenbeeTask):
    ...
    default_states = luigi.Parameter(default='all')
    ...
    @property
    def __script__(self):
        return pathlib.Path(__file__).parent.joinpath('scripts', 'prepare_multiphase.py').resolve()
    ...
    def output(self):
        return {
            ...
            'two_phase_info': luigi.LocalTarget(
                pathlib.Path(self.execution_folder, 'resources/two_phase.json').resolve().as_posix()
            ),
    ...
    def output_artifacts(self):
        return [
            ...
            {
                'name': 'two-phase-info', 'from': 'two_phase.json',
                'to': pathlib.Path(self.execution_folder, 'resources/two_phase.json').resolve().as_posix(),
                'optional': False,
                'type': 'file'
            },
...
class _TwoPhasePrepareFolder_52a459c9Orchestrator(luigi.WrapperTask):
    ...
    def requires(self):
        yield [CreateDirectSky(_input_params=self.input_values), CreateSkyDome(_input_params=self.input_values), CreateStudyInfo(_input_params=self.input_values), CreateTotalSky(_input_params=self.input_values), ParseSunUpHours(_input_params=self.input_values), PrepareMultiphase(_input_params=self.input_values)]
