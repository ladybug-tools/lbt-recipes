...
class GetPrevailingTemperature(QueenbeeTask):
    ...
    def command(self):
        return 'ladybug-comfort epw prevailing weather.epw --comfort-par "{comfort_par}" --run-period "{run_period}" --{output_format} --{order_by} --output-file prevailing.csv'.format(comfort_par=self.comfort_par, output_format=self.output_format, order_by=self.order_by, run_period=self.run_period)
...
class CreateDirectSky(QueenbeeTask):
    ...
    def command(self):
        return 'honeybee-radiance sky mtx sky.epw --name sky --north {north} --sky-type {sky_type} --{cumulative} --{sun_up_hours} --{output_type} --output-format {output_format} --sky-density {sky_density}'.format(sky_density=self.sky_density, sun_up_hours=self.sun_up_hours, output_type=self.output_type, sky_type=self.sky_type, output_format=self.output_format, cumulative=self.cumulative, north=self.north)
...
class _PrepareFolder_6b5ce744Orchestrator(luigi.WrapperTask):
    ...
    def requires(self):
        yield [CreateModelOccSchedules(_input_params=self.input_values), CreateModelTransSchedules(_input_params=self.input_values), CreateSkyDome(_input_params=self.input_values), CreateViewFactorModifiers(_input_params=self.input_values), GetPrevailingTemperature(_input_params=self.input_values), CreateDirectSky(_input_params=self.input_values), CreateTotalSky(_input_params=self.input_values), CopyGridInfo(_input_params=self.input_values), CreateDynamicOctrees(_input_params=self.input_values), CreateOctree(_input_params=self.input_values), CreateOctreeWithSuns(_input_params=self.input_values), CreateShadeTransOctrees(_input_params=self.input_values), ParseSunUpHours(_input_params=self.input_values), CopyRedistInfo(_input_params=self.input_values)]
