...
from .dependencies.energy_simulation import _EnergySimulation_6b5ce744Orchestrator as EnergySimulation_6b5ce744Workerbee
from .dependencies.prepare_folder import _PrepareFolder_6b5ce744Orchestrator as PrepareFolder_6b5ce744Workerbee
from .dependencies.comfort_mapping_entry_point import _ComfortMappingEntryPoint_6b5ce744Orchestrator as ComfortMappingEntryPoint_6b5ce744Workerbee
from .dependencies.dynamic_contribution_entry_point import _DynamicContributionEntryPoint_6b5ce744Orchestrator as DynamicContributionEntryPoint_6b5ce744Workerbee
from .dependencies.dynamic_shade_contrib_entry_point import _DynamicShadeContribEntryPoint_6b5ce744Orchestrator as DynamicShadeContribEntryPoint_6b5ce744Workerbee
from .dependencies.radiance_mapping_entry_point import _RadianceMappingEntryPoint_6b5ce744Orchestrator as RadianceMappingEntryPoint_6b5ce744Workerbee
from .dependencies.spherical_view_factor_entry_point import _SphericalViewFactorEntryPoint_6b5ce744Orchestrator as SphericalViewFactorEntryPoint_6b5ce744Workerbee
...
class EnergySimulation(QueenbeeTask):
    ...
    def run(self):
        yield [EnergySimulation_6b5ce744Workerbee(_input_params=self.map_dag_inputs)]
...
class PrepareFolder(QueenbeeTask):
    ...
    def run(self):
        yield [PrepareFolder_6b5ce744Workerbee(_input_params=self.map_dag_inputs)]
        pathlib.Path(self.execution_folder).mkdir(parents=True, exist_ok=True)
        self._copy_output_artifacts(self.execution_folder)
        self._copy_output_parameters(self.execution_folder)
...
class RunRadianceSimulationLoop(luigi.Task):
    ...
    def run(self):
        yield [RadianceMappingEntryPoint_6b5ce744Workerbee(_input_params=self.map_dag_inputs)]
...
class RunSphericalViewFactorSimulationLoop(luigi.Task):
    ...
    def run(self):
        yield [SphericalViewFactorEntryPoint_6b5ce744Workerbee(_input_params=self.map_dag_inputs)]
...
class RunRadianceDynamicContributionLoop(luigi.Task):
    ...
    def run(self):
        yield [DynamicContributionEntryPoint_6b5ce744Workerbee(_input_params=self.map_dag_inputs)]
...
class RunRadianceShadeContributionLoop(luigi.Task):
    ...
    def run(self):
        yield [DynamicShadeContribEntryPoint_6b5ce744Workerbee(_input_params=self.map_dag_inputs)]
...
class RunComfortMapLoop(luigi.Task):
    ...
    @property
    def prevailing(self):
        value = pathlib.Path(self.input()['PrepareFolder']['initial_results'].path, 'conditions/prevailing.csv')
        return value.as_posix() if value.is_absolute() \
            else pathlib.Path(self.initiation_folder, value).resolve().as_posix()
    ...
    def run(self):
        yield [ComfortMappingEntryPoint_6b5ce744Workerbee(_input_params=self.map_dag_inputs)]
...
class _Main_6b5ce744Orchestrator(luigi.WrapperTask):
