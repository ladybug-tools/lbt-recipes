...
import flow.main_0c3ede2f as leed_daylight_option_two_workerbee
...
class LetLeedDaylightOptionTwoFly(luigi.WrapperTask):
    # global parameters
    _input_params = luigi.DictParameter()

    def requires(self):
        yield [leed_daylight_option_two_workerbee._Main_0c3ede2fOrchestrator(_input_params=self._input_params)]
...
def start(project_folder, user_values, workers):
    ...
    if 'simulation_folder' not in input_params or not input_params['simulation_folder']:
        if 'simulation_id' not in input_params or not input_params['simulation_id']:
            simulation_id = 'leed_daylight_option_two_%d' % int(round(time.time(), 2) * 100)
        else:
    ...
    summary = luigi.build(
        [LetLeedDaylightOptionTwoFly(_input_params=input_params)],
