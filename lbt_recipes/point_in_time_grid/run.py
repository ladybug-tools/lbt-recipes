...
import flow.main_ca165f46 as point_in_time_grid_workerbee
...
class LetPointInTimeGridFly(luigi.WrapperTask):
    ...
    def requires(self):
        yield [point_in_time_grid_workerbee._Main_ca165f46Orchestrator(_input_params=self._input_params)]
...
def start(project_folder, user_values, workers):
    ...
    if 'simulation_folder' not in input_params or not input_params['simulation_folder']:
        if 'simulation_id' not in input_params or not input_params['simulation_id']:
            simulation_id = 'point_in_time_grid_%d' % int(round(time.time(), 2) * 100)
        else:
    ...
    summary = luigi.build(
        [LetPointInTimeGridFly(_input_params=input_params)],
