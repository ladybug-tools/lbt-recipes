...
class CreateRadFolder(QueenbeeTask):
    ...
    @property
    def task_image(self):
        return 'docker.io/ladybugtools/honeybee-radiance:1.64.8'
...
class CreateSky(QueenbeeTask):
    ...
    def command(self):
        return 'honeybee-radiance sky mtx sky.wea --name sky --north {north} --sky-type {sky_type} --{cumulative} --{sun_up_hours} --{output_type} --output-format {output_format} --sky-density {sky_density}'.format(sky_type=self.sky_type, sky_density=self.sky_density, output_type=self.output_type, cumulative=self.cumulative, sun_up_hours=self.sun_up_hours, north=self.north, output_format=self.output_format)
...
class SkyRadiationRaytracingLoop(QueenbeeTask):
    ...
    def command(self):
        return 'honeybee-radiance dc scoeff scene.oct grid.pts sky.dome sky.mtx --sensor-count {sensor_count} --output results.ill --rad-params "{radiance_parameters}" --rad-params-locked "{fixed_radiance_parameters}" --conversion "{conversion}" --output-format {output_format} --order-by-{order_by} --{header}-header'.format(sensor_count=self.sensor_count, radiance_parameters=self.radiance_parameters, conversion=self.conversion, order_by=self.order_by, fixed_radiance_parameters=self.fixed_radiance_parameters, output_format=self.output_format, header=self.header)
...
class _Main_62668fa9Orchestrator(luigi.WrapperTask):
    ...
    def requires(self):
        yield [CopyGridInfo(_input_params=self.input_values), AccumulateResults(_input_params=self.input_values)]
