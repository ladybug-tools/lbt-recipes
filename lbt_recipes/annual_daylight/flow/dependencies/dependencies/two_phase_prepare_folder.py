...
class CreateDirectSky(QueenbeeTask):
    ...
    def command(self):
        return 'honeybee-radiance sky mtx sky.wea --name sky --north {north} --sky-type {sky_type} --{cumulative} --{sun_up_hours} --{output_type} --output-format {output_format} --sky-density {sky_density}'.format(sky_type=self.sky_type, cumulative=self.cumulative, sky_density=self.sky_density, sun_up_hours=self.sun_up_hours, north=self.north, output_type=self.output_type, output_format=self.output_format)
...
class _TwoPhasePrepareFolder_acc750a6Orchestrator(luigi.WrapperTask):
    ...
    def requires(self):
        yield [CreateDirectSky(_input_params=self.input_values), CreateSkyDome(_input_params=self.input_values), CreateTotalSky(_input_params=self.input_values), ParseSunUpHours(_input_params=self.input_values), PrepareMultiphase(_input_params=self.input_values)]
