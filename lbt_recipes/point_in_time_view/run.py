...
import flow.main_2514a5fc as point_in_time_view_workerbee
...
class LetPointInTimeViewFly(luigi.WrapperTask):
    # global parameters
    _input_params = luigi.DictParameter()

    def requires(self):
        yield [point_in_time_view_workerbee._Main_2514a5fcOrchestrator(_input_params=self._input_params)]
...
def start(project_folder, user_values, workers):
    ...
    if 'simulation_folder' not in input_params or not input_params['simulation_folder']:
        if 'simulation_id' not in input_params or not input_params['simulation_id']:
            simulation_id = 'point_in_time_view_%d' % int(round(time.time(), 2) * 100)
        else:
