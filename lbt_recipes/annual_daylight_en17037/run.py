...
import flow.main_d9049334 as annual_daylight_en17037_workerbee
...
class LetAnnualDaylightEn17037Fly(luigi.WrapperTask):
    # global parameters
    _input_params = luigi.DictParameter()

    def requires(self):
        yield [annual_daylight_en17037_workerbee._Main_d9049334Orchestrator(_input_params=self._input_params)]
...
def start(project_folder, user_values, workers):
    ...
    if 'simulation_folder' not in input_params or not input_params['simulation_folder']:
        if 'simulation_id' not in input_params or not input_params['simulation_id']:
            simulation_id = 'annual_daylight_en17037_%d' % int(round(time.time(), 2) * 100)
        else:
    ...
    artifacts = ['epw', 'grid_metrics', 'model']
    ...
    summary = luigi.build(
        [LetAnnualDaylightEn17037Fly(_input_params=input_params)],
