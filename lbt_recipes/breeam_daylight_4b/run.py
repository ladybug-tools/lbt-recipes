...
import flow.main_0eec764c as breeam_daylight_4b_workerbee
...
class LetBreeamDaylight4bFly(luigi.WrapperTask):
    # global parameters
    _input_params = luigi.DictParameter()

    def requires(self):
        yield [breeam_daylight_4b_workerbee._Main_0eec764cOrchestrator(_input_params=self._input_params)]
...
def start(project_folder, user_values, workers):
    ...
    if 'simulation_folder' not in input_params or not input_params['simulation_folder']:
        if 'simulation_id' not in input_params or not input_params['simulation_id']:
            simulation_id = 'breeam_daylight_4b_%d' % int(round(time.time(), 2) * 100)
        else:
    ...
    summary = luigi.build(
        [LetBreeamDaylight4bFly(_input_params=input_params)],
