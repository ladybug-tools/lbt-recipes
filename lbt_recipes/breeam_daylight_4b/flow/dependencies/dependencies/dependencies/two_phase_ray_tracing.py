...
class DirectSunlight(QueenbeeTask):
    ...
    def command(self):
        return 'honeybee-radiance dc scontrib scene.oct grid.pts suns.mod --{calculate_values} --sensor-count {sensor_count} --rad-params "{radiance_parameters}" --rad-params-locked "{fixed_radiance_parameters}" --conversion "{conversion}" --output-format {output_format} --output results.ill --order-by-{order_by} --{header}-header'.format(radiance_parameters=self.radiance_parameters, sensor_count=self.sensor_count, output_format=self.output_format, calculate_values=self.calculate_values, conversion=self.conversion, order_by=self.order_by, fixed_radiance_parameters=self.fixed_radiance_parameters, header=self.header)
...
class _TwoPhaseRayTracing_c3773f23Orchestrator(luigi.WrapperTask):
