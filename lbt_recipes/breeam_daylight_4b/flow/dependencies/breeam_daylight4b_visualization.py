...
class CopyPassFail(QueenbeeTask):
    ...
    @property
    def src(self):
        value = pathlib.Path(self._input_params['pass_fail'])
    ...
    @property
    def __script__(self):
        return pathlib.Path(__file__).parent.joinpath('scripts', 'copy_pass_fail.py').resolve()
    ...
    def output(self):
        return {
            'dst': luigi.LocalTarget(
                pathlib.Path(self.execution_folder, 'visualization/pass_fail').resolve().as_posix()
            )
        }
        ...
        return [
            {
                ...
                'to': pathlib.Path(self.execution_folder, 'visualization/pass_fail').resolve().as_posix(),
                'optional': False,
                'type': 'folder'
            }]
...
class CreateVisMetadata(QueenbeeTask):
    ...
    def command(self):
        return 'honeybee-radiance-postprocess post-process breeam breeam-4b-vis-metadata --output-folder "{output_folder}"'.format(output_folder=self.output_folder)
...
class CreateVsf(QueenbeeTask):
    ...
    def requires(self):
        return {'CopyPassFail': CopyPassFail(_input_params=self._input_params), 'CreateVisMetadata': CreateVisMetadata(_input_params=self._input_params)}
...
class _BreeamDaylight4bVisualization_d1bca667Orchestrator(luigi.WrapperTask):
