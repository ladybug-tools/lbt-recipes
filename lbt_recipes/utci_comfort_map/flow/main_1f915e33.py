...
class PrepareFolder(QueenbeeTask):
    ...
    def map_dag_inputs(self):
        ...
        inputs = {
            ...
            'air_speed_matrices': self.air_speed_matrices
        }
        try:
            inputs['__debug__'] = self._input_params['__debug__']
        except KeyError:
...
class RunRadianceDynamicShadeContributionLoop(luigi.Task):
    ...
    sensor_grids = luigi.Parameter(default='[]')
    ...
    def sensor_grids(self):
        value = pathlib.Path(self.input()['PrepareFolder']['sensor_grids_folder'].path, '_split_info.json')
        return value.as_posix() if value.is_absolute() \
            else pathlib.Path(self.initiation_folder, value).resolve().as_posix()
...
class RunComfortMapLoop(luigi.Task):
    ...
    def result_sql(self):
        try:
            pathlib.Path(self.input()['EnergySimulation']['energy'].path, 'eplusout.sql')
        except TypeError:
            # optional artifact
            return None
