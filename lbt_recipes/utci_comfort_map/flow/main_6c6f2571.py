...
from .dependencies.comfort_mapping_entry_point import _ComfortMappingEntryPoint_6c6f2571Orchestrator as ComfortMappingEntryPoint_6c6f2571Workerbee
from .dependencies.dynamic_shade_contrib_entry_point import _DynamicShadeContribEntryPoint_6c6f2571Orchestrator as DynamicShadeContribEntryPoint_6c6f2571Workerbee
from .dependencies.radiance_mapping_entry_point import _RadianceMappingEntryPoint_6c6f2571Orchestrator as RadianceMappingEntryPoint_6c6f2571Workerbee
...
class CreateResultInfo(QueenbeeTask):
    ...
    def command(self):
        return 'ladybug-comfort map map-result-info {comfort_model} --run-period "{run_period}" --qualifier "{qualifier}" --folder output --log-file results_info.json'.format(run_period=self.run_period, comfort_model=self.comfort_model, qualifier=self.qualifier)
...
class CreateSimPar(QueenbeeTask):
    ...
    def command(self):
        return 'honeybee-energy settings comfort-sim-par input.ddy --run-period "{run_period}" --north {north} --{filter_des_days} --output-file sim_par.json'.format(filter_des_days=self.filter_des_days, run_period=self.run_period, north=self.north)
...
class SetModifiersFromConstructions(QueenbeeTask):
    ...
    def command(self):
        return 'honeybee-energy edit modifiers-from-constructions model.hbjson --{use_visible} --{dynamic_behavior}-groups --{dynamic_shade}-groups --exterior-offset {exterior_offset} --output-file new_model.hbjson'.format(exterior_offset=self.exterior_offset, use_visible=self.use_visible, dynamic_shade=self.dynamic_shade, dynamic_behavior=self.dynamic_behavior)
...
class SplitAirSpeedFolder(QueenbeeTask):
    ...
    def command(self):
        return 'honeybee-radiance grid split-folder ./input_folder ./output_folder {cpu_count} {extension} --grid-divisor {cpus_per_grid} --min-sensor-count {min_sensor_count} --grid-info-file grid_info.json'.format(cpu_count=self.cpu_count, min_sensor_count=self.min_sensor_count, extension=self.extension, cpus_per_grid=self.cpus_per_grid)
...
class RunRadianceSimulationLoop(luigi.Task):
    ...
    def run(self):
        yield [RadianceMappingEntryPoint_6c6f2571Workerbee(_input_params=self.map_dag_inputs)]
...
class RunRadianceDynamicShadeContributionLoop(luigi.Task):
    """No description is provided."""
    ...
    @property
    def octree_file(self):
        value = pathlib.Path(self.input()['CreateDynamicShadeOctrees']['scene_folder'].path, self.item['default'])
        return value.as_posix() if value.is_absolute() \
            else pathlib.Path(self.initiation_folder, value).resolve().as_posix()

    @property
    def octree_file_with_suns(self):
        value = pathlib.Path(self.input()['CreateDynamicShadeOctrees']['scene_folder'].path, self.item['sun'])
        return value.as_posix() if value.is_absolute() \
            else pathlib.Path(self.initiation_folder, value).resolve().as_posix()
    ...
    def run(self):
        yield [DynamicShadeContribEntryPoint_6c6f2571Workerbee(_input_params=self.map_dag_inputs)]
        done_file = pathlib.Path(self.execution_folder, 'run_radiance_dynamic_shade_contribution.done')
        done_file.parent.mkdir(parents=True, exist_ok=True)
        done_file.write_text('done!')

    def requires(self):
        return {'CreateSkyDome': CreateSkyDome(_input_params=self._input_params), 'GenerateSunpath': GenerateSunpath(_input_params=self._input_params), 'ParseSunUpHours': ParseSunUpHours(_input_params=self._input_params), 'CreateTotalSky': CreateTotalSky(_input_params=self._input_params), 'CreateDirectSky': CreateDirectSky(_input_params=self._input_params), 'SplitGridFolder': SplitGridFolder(_input_params=self._input_params), 'CreateDynamicShadeOctrees': CreateDynamicShadeOctrees(_input_params=self._input_params), 'RunRadianceSimulation': RunRadianceSimulation(_input_params=self._input_params)}

    def output(self):
        return {
            'is_done': luigi.LocalTarget(pathlib.Path(self.execution_folder, 'run_radiance_dynamic_shade_contribution.done').resolve().as_posix())
        }


class RunRadianceDynamicShadeContribution(luigi.Task):
    """No description is provided."""
    # global parameters
    _input_params = luigi.DictParameter()
    @property
    def scene_info(self):
        value = pathlib.Path(self.input()['CreateDynamicShadeOctrees']['scene_info'].path)
        return value.as_posix() if value.is_absolute() \
            else pathlib.Path(self.initiation_folder, value).resolve().as_posix()
    ...
    def items(self):
        try:
            ...
            # it is a parameter
            return self.input()['CreateDynamicShadeOctrees']['scene_info'].path

    def run(self):
        yield [RunRadianceDynamicShadeContributionLoop(item=item, _input_params=self._input_params) for item in self.items]
        done_file = pathlib.Path(self.execution_folder, 'run_radiance_dynamic_shade_contribution.done')
        done_file.parent.mkdir(parents=True, exist_ok=True)
        done_file.write_text('done!')
...
class RunComfortMapLoop(luigi.Task):
    ...
    comfort_parameters = luigi.Parameter(default='--cold 9 --heat 26')

    solarcal_parameters = luigi.Parameter(default='--posture standing --sharp 135 --absorptivity 0.7 --emissivity 0.95')
    ...
    def result_sql(self):
        try:
            pathlib.Path(self.input()['RunEnergySimulation']['sql'].path)
        except TypeError:
            # optional artifact
            return None
...
class _Main_6c6f2571Orchestrator(luigi.WrapperTask):
