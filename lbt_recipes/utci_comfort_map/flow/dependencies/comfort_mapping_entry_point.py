...
class ProcessUtciMatrix(QueenbeeTask):
    ...
    def command(self):
        return 'ladybug-comfort mtx utci air_temperature.csv rel_humidity.csv --rad-temperature-mtx rad_temperature.csv --rad-delta-mtx rad_delta.csv --wind-speed-json wind_speed.json --air-speed-mtx air_speed.csv --comfort-par "{comfort_par}" --{output_format} --folder output'.format(comfort_par=self.comfort_par, output_format=self.output_format)
