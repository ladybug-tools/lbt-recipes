...
_default_inputs = {   'l01_pass_fail': None,
    'l06_pass_fail': None,
    'model': None,
...
class CopyL01PassFail(QueenbeeTask):
    ...
    @property
    def src(self):
        value = pathlib.Path(self._input_params['l01_pass_fail'])
    ...
    @property
    def __script__(self):
        return pathlib.Path(__file__).parent.joinpath('scripts', 'copy_l01_pass_fail.py').resolve()
    ...
    def output(self):
        return {
            'dst': luigi.LocalTarget(
                pathlib.Path(self.execution_folder, 'visualization/L01').resolve().as_posix()
            )
        }
        ...
        return [
            {
                ...
                'to': pathlib.Path(self.execution_folder, 'visualization/L01').resolve().as_posix(),
                'optional': False,
                'type': 'folder'
            }]
...
class CopyL06PassFail(QueenbeeTask):
    """Copy a folder to a destination."""
    ...
    @property
    def src(self):
        value = pathlib.Path(self._input_params['l06_pass_fail'])
        return value.as_posix() if value.is_absolute() \
            else pathlib.Path(self.initiation_folder, value).resolve().as_posix()
    ...
    @property
    def __script__(self):
        return pathlib.Path(__file__).parent.joinpath('scripts', 'copy_l06_pass_fail.py').resolve()
    ...
    def output(self):
        return {
            'dst': luigi.LocalTarget(
                pathlib.Path(self.execution_folder, 'visualization/L06').resolve().as_posix()
            )
        }
    ...
    def output_artifacts(self):
        return [
            {
                'name': 'dst', 'from': 'input.path',
                'to': pathlib.Path(self.execution_folder, 'visualization/L06').resolve().as_posix(),
                'optional': False,
                'type': 'folder'
            }]
...
class CreateVisMetadata(QueenbeeTask):
    ...
    def command(self):
        return 'honeybee-radiance-postprocess post-process well well-daylight-vis-metadata --output-folder "{output_folder}"'.format(output_folder=self.output_folder)
...
class CreateVsf(QueenbeeTask):
    ...
    def command(self):
        return 'honeybee-display model-to-vis model.hbjson --color-by {color_by} --{color_visibility}-color-by --{wireframe} --{attr_format}-attr --room-attr "{room_attr}" --face-attr "{face_attr}" --grid-display-mode {grid_display_mode} --{grid_visibility}-grid --grid-data input_data --grid-data-display-mode {grid_data_display_mode} --active-grid-data "{active_grid_data}" --output-format {output_format} --output-file model_vis.{output_format}'.format(wireframe=self.wireframe, room_attr=self.room_attr, grid_display_mode=self.grid_display_mode, face_attr=self.face_attr, active_grid_data=self.active_grid_data, color_by=self.color_by, output_format=self.output_format, grid_data_display_mode=self.grid_data_display_mode, grid_visibility=self.grid_visibility, color_visibility=self.color_visibility, attr_format=self.attr_format)
...
class _WellDaylightVisualization_1d5ef609Orchestrator(luigi.WrapperTask):
