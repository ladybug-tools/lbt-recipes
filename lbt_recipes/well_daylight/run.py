...
import flow.main_f1842a5f as well_daylight_workerbee
...
class LetWellDaylightFly(luigi.WrapperTask):
    # global parameters
    _input_params = luigi.DictParameter()

    def requires(self):
        yield [well_daylight_workerbee._Main_f1842a5fOrchestrator(_input_params=self._input_params)]
...
def start(project_folder, user_values, workers):
    ...
    if 'simulation_folder' not in input_params or not input_params['simulation_folder']:
        if 'simulation_id' not in input_params or not input_params['simulation_id']:
            simulation_id = 'well_daylight_%d' % int(round(time.time(), 2) * 100)
        else:
    ...
    summary = luigi.build(
        [LetWellDaylightFly(_input_params=input_params)],
