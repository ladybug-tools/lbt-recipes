...
class ConvertWeaToConstant(QueenbeeTask):
    ...
    @property
    def __script__(self):
        return pathlib.Path(__file__).parent.joinpath('scripts', 'convert_wea_to_constant.py').resolve()
...
class WriteTimestep(QueenbeeTask):
    ...
    @property
    def __script__(self):
        return pathlib.Path(__file__).parent.joinpath('scripts', 'write_timestep.py').resolve()
...
class _DirectSunHoursPrepareFolder_6aed763bOrchestrator(luigi.WrapperTask):
