...
class ComputeTcp(QueenbeeTask):
    ...
    def condition_csv(self):
        value = pathlib.Path(self.input()['ProcessPmvMatrix']['condition_map'].path)
    ...
    def requires(self):
        return {'ProcessPmvMatrix': ProcessPmvMatrix(_input_params=self._input_params)}
...
class ProcessPmvMatrix(QueenbeeTask):
    ...
    def command(self):
        return 'ladybug-comfort mtx pmv air_temperature.csv rel_humidity.csv --rad-temperature-mtx rad_temperature.csv --rad-delta-mtx rad_delta.csv --air-speed-json air_speed.json --met-rate "{met_rate}" --clo-value "{clo_value}" --comfort-par "{comfort_par}" --{write_set_map} --folder output'.format(met_rate=self.met_rate, clo_value=self.clo_value, comfort_par=self.comfort_par, write_set_map=self.write_set_map)
...
class _ComfortMappingEntryPoint_1c5c7571Orchestrator(luigi.WrapperTask):
